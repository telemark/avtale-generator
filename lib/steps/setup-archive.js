...
module.exports = async data => {
  logger('info', ['setup-archive', data._id, data.agreementTemplate])
  const now = new Date()
  ...
  if (data.missingGuardianDocumentData !== false) {
    const manuellTemplate = getTemplate({ domain: 'avtaler', templateId: 'manuell' })
  ...
  if (data.restrictedDocumentData !== false) {
    const restrictedTemplate = getTemplate({ domain: 'avtaler', templateId: 'restricted' })

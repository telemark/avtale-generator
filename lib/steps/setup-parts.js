...
module.exports = data => {
  return new Promise(async (resolve, reject) => {
    ...
    if (data.dsf !== false) {
      ...
      if (data.p360Elevmappe && data.p360Elevmappe !== false) {
        const caseNumberSplit = data.p360Elevmappe.CaseNumber.split('/')
        distribution.metadataForImport = {
          dokumentetsDato: new Date(Date.now()).toISOString(),
          journalposttype: 'Dokument inn',
          journalstatus: 'J',
          saksaar: caseNumberSplit[0],
          sakssekvensnummer: caseNumberSplit[1],
          tittel: title
        }
      }
